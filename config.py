...
# Reusable validator functions to avoid duplication
def validate_log_channel_map(v: Any) -> Dict[int, int]:
    """Parse LOG_CHANNEL_MAP from string or dict format."""
    if v is None or v == "":
        return {}
...
def validate_default_purge_count(v: int) -> int:
    """Validate DEFAULT_PURGE_COUNT is positive."""
    if v <= 0:
...
class TelegramConfig(BaseConfig):
    ...
    def parse_log_channel_map(cls, v):
        return validate_log_channel_map(v)

    @field_validator('default_purge_count')
    @classmethod
    def _validate_default_purge_count(cls, v):
        return validate_default_purge_count(v)
...
class Config(BaseConfig):
    ...
    def for_testing(cls, **overrides) -> 'Config':
        ...
        for key, value in overrides.items():
            ...
            elif key == 'LOG_CHANNEL_MAP':
                key = 'log_channel_map'
            elif key == 'DEFAULT_PURGE_COUNT':
                key = 'default_purge_count'
            defaults[key] = value
